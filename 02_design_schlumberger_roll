...
def pseudo_section_with_depth(data, other_array=False, array=None, array_type='dd', interp_method='linear', scatter=True, cmin=None, cmax=None):
    ...
    for i in range(nconfig):
        ...
        elif array_type == 'wa' or array_type == 'slm':  # slm should have its own ze_over_a
            a_dist = x_p1 - x_c1
            ze_over_a = 0.519
            ze = -ze_over_a * a_dist
            xe = (x_c1 + x_c2 + x_p1 + x_p2) / 4
...
end_line = 143
ne = 144
scheme_ref = pb.createData(elecs=pg.utils.grange(start=x_ini, end=end_line, n=ne), schemeName='slm')
dataformatsyscal(scheme_ref, name='schlumberger_reference.txt')
...
end_line_roll = 179
ne = 144
scheme_roll = pb.createData(elecs=pg.utils.grange(start=x_ini_roll, end=end_line_roll, n=ne), schemeName='slm')
nconfig_roll = scheme_roll.size()
pseudo_section_with_depth(scheme_ref, array_type='slm')
...
pseudo_section_with_depth(scheme_roll, array_type='slm')
dataformatsyscal(scheme_roll, name='schlumberger_roll.txt')
